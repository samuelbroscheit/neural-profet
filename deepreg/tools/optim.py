...
class OptimRegime(object):
    ...
    def __init__(self, params, regime, filter_weight_decay=False):
        self.optimizer = torch.optim.Adam(params, lr=0)
        if filter_weight_decay:
            regime = [{k:v for k,v in step.items() if k!='weight_decay'} for step in regime]
        self.regime = regime
